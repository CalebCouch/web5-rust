...
use super::did_dht::{DidDht, Type, Key, Purpose};
...
impl DhtDns {
    ...
    pub fn to_bytes(dht: &DidDht, gateways: Vec<Url>) -> Result<Vec<u8>, Error> {
        ...
        for (index, key) in keys.iter().enumerate() {
            ...
            let (t, a) = Self::key_to_ta(&key.public_key);
        ...
        for (index, service) in dht.services.iter().enumerate() {
            ...
            ss.insert("t".to_string(), service.r#type.join(VALUE_SEPARATOR));
            ss.insert(
                "se".to_string(),
    ...
    pub fn from_bytes(packet: &[u8], id: String) -> Result<(), Error> {
